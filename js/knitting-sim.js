(function () {
  ...
  // ═══════════════════════════════════════════
  // DATA — from knitting-data.js (50 notes, 8 threads)
  // ═══════════════════════════════════════════
  ...
  // precompute note→thread mapping
  var noteThreads = [];
  for (var i = 0; i < NOTES.length; i++) noteThreads.push([]);
  THREADS.forEach(function (th, ti) {
    th.ids.forEach(function (id) {
      if (noteThreads[id].indexOf(ti) === -1) noteThreads[id].push(ti);
    });
  });

  // ═══════════════════════════════════════════
  // Section 2: Build month cards
  // ═══════════════════════════════════════════
  ...
  function buildMonths() {
    ...
    buckets.forEach(function (ids, m) {
      ...
      ids.forEach(function (idx) {
        ...
        var isCross = threads.length > 1;

        var card = document.createElement('div');
        card.className = 'ks-card' + (isCross ? ' ks-card--cross' : '');
        ...
        if (isCross) {
          ...
          knot.style.setProperty('--kc1', c1);
          knot.style.setProperty('--kc2', c2);
  ...
  function buildThreads() {
    ...
    THREADS.forEach(function (th, ti) {
      ...
      head.innerHTML = '<span class="ks-thread__color" style="background:' + th.c + '"></span>' +
                        '<span class="ks-thread__name">' + esc(th.n) + '</span>';
      div.appendChild(head);

      var items = document.createElement('div');
      items.className = 'ks-thread__items';
      items.style.cssText = '--tc:' + th.c;
      // vertical line color
      ...
      sorted.forEach(function (idx) {
        ...
        // other threads this note belongs to
  ...
  function drawOverview(canvasId) {
    ...
    buckets.forEach(function (ids, m) {
      ...
      ids.forEach(function (idx, j) {
        ...
        pos[idx] = {
          x: cx + (srand(idx) - 0.5) * (usableW / 16),
          y: padY + spread * usableH
        };
      });
    });
    ...
    THREADS.forEach(function (th, ti) {
      ...
      for (var k = 1; k < sorted.length; k++) {
        var p0 = pos[sorted[k - 1]];
        var p1 = pos[sorted[k]];
        var cpx = (p0.x + p1.x) / 2;
        var cpy = (p0.y + p1.y) / 2 + (srand(ti * 100 + k) - 0.5) * 40;
    ...
    NOTES.forEach(function (n, i) {
      var p = pos[i];
      var threads = noteThreads[i];
      var r = threads.length > 1 ? 3.5 : 2;
      ctx.beginPath();
      ctx.arc(p.x, p.y, r, 0, Math.PI * 2);
      if (threads.length > 0) {
        ctx.fillStyle = hexAlpha(THREADS[threads[0]].c, 0.5);
      } else {
  ...
  function setupObserver() {
    ...
    document.querySelectorAll('.ks-card, .ks-thread__item').forEach(function (el) {
      observer.observe(el);
    });
  }

  // ═══════════════════════════════════════════
  // Knot icon CSS injection
  // ═══════════════════════════════════════════

  function injectKnotStyles() {
    var style = document.createElement('style');
    style.textContent =
      '.ks-card__knot::before { background: var(--kc1, #c8a96e); }' +
      '.ks-card__knot::after { background: var(--kc2, #7db8d4); }';
    document.head.appendChild(style);
  }
  ...
  function srand(seed) {
    var x = Math.sin(seed * 9301 + 49297) * 49271;
