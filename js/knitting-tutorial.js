/**
 * SensePoint — Knitting Tutorial
 * 뜨개질 안내 페이지의 인터랙티브 체험 기능
 */
...
(function () {
  ...
  function goStep(idx) {
    ...
    $progLines.forEach(function (el, i) {
      el.classList.toggle('is-filled', i < idx);
    });
  ...
  function updateNextBtn() {
    ...
    if (currentStep === totalSteps - 1 && done) {
      $next.textContent = 'My Note에서 시작하기';
  ...
  $next.addEventListener('click', function () {
    if (currentStep === totalSteps - 1 && stepCompleted[currentStep]) {
      window.location.href = 'my-note.html';
      return;
    }
  ...
  function renderCluePath() {
    if (!$cluePath) return;
    var names = [];
    document.querySelectorAll('.tut-clue-note.is-connected h4').forEach(function (h) {
      names.push(h.textContent);
    });
  ...
  function buildResultSection() {
    ...
    var crossCount = 0;
    for (var j = 0; j < noteThreads.length; j++) {
      if (noteThreads[j].length >= 2) crossCount++;
    }

    var $statThreads = document.getElementById('resultStatThreads');
    var $statStitches = document.getElementById('resultStatStitches');
    var $statCross = document.getElementById('resultStatCross');
    if ($statThreads) $statThreads.textContent = threads.length;
    if ($statStitches) $statStitches.textContent = notes.length;
    if ($statCross) $statCross.textContent = crossCount;
    ...
    $canvas.innerHTML = '';
    resultDisplayNoteIds.forEach(function (noteId, idx) {
      var n = notes[noteId];
      if (!n) return;
  ...
  function drawResultLines() {
    ...
    nodes.forEach(function (n) {
      ...
      positions.push({
        ...
        y: nr.top - rect.top + nr.height / 2 - 10
      });
    });
    ...
    threads.forEach(function (th, ti) {
      ...
      th.ids.forEach(function (noteId) {
        ...
        if (prevIdx >= 0) {
          var a = positions[prevIdx];
          var b = positions[idx];
          var offset = (ti - 4) * 5;

          var path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
          var mx = (a.x + b.x) / 2;
          var my = (a.y + b.y) / 2 + offset * 2;
          path.setAttribute('d',
            'M' + a.x + ',' + (a.y + offset) +
            ' Q' + mx + ',' + (my + offset) +
            ' ' + b.x + ',' + (b.y + offset));
  ...
  window.addEventListener('resize', function () {
    if (currentStep === 3) drawResultLines();
  });
